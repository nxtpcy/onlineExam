...
public class RedisCacheManager implements CacheManager {
	...
	public <K, V> Cache<K, V> getCache(String arg0) throws CacheException {
		...
		return new ShiroRedisCache<K, V>(cacheKeyPrefix + arg0);
	}
	...
	public class ShiroRedisCache<K, V> implements Cache<K, V> {
		...
		protected Object hashKey(K key) {
			...
			if (key instanceof PrincipalCollection) {// 此处很重要,如果key是登录凭证,那么这是访问用户的授权缓存;将登录凭证转为user对象,返回user的id属性做为hash
